use graphdener::{Transaction, Datastore, VertexQuery, Vertex, Error};
use statics;
use uuid::Uuid;
...
{
	let trans = statics::DATASTORE.transaction().unwrap();
	trans.get_vertices(&VertexQuery::All{ start_id: None, limit: LIMIT })
}

pub fn get_vertex_neighbors(uuid: Uuid) -> Result<Vec<Vertex>, Error>
{
	let trans = statics::DATASTORE.transaction().unwrap();
	trans.get_vertices(&VertexQuery::Vertices{ ids: vec!(uuid) }.outbound_edges(None, None, None, None, EDGE_LIMIT).inbound_vertices(EDGE_LIMIT))
}

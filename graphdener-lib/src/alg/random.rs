...
pub fn random_uniform(graph: &mut Graph, min_distance: f64, spread_factor: u8) -> &str {
    ...
    let positionx: Array<f64> =
        randu::<f64>(Dim4::new(&[1, n as u64, 1, 1])) * l.ln() * spread_factor;
...
pub fn random_normalized(graph: &mut Graph, min_distance: f64, spread_factor: u8) -> &str {
    fn distance(p1: (f64, f64), p2: (f64, f64)) -> f64 {
        ((p1.0 - p2.0).powf(2.) + (p1.1 - p2.1).powf(2.)).sqrt()
    }
    let n = graph.count();
    ...
    let positionx: Array<f64> =
        randn::<f64>(Dim4::new(&[1, n as u64, 1, 1])) * l.ln() * spread_factor;
    let positiony: Array<f64> =
        randn::<f64>(Dim4::new(&[1, n as u64, 1, 1])) * l.ln() * spread_factor;
